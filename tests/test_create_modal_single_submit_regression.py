...
# feature in #426, and the runtime-profile modal went when each member got a
# single settings row (Connectors); only the create-assistant modal remains.
def test_create_modal_submit_handlers_use_begin_single_submit():
    js = Path("app/static/js/chat_ui.js").read_text(encoding="utf-8")

    assert "closeCreateRuntimeProfileModal" not in js
...
def test_create_modal_close_handlers_check_submitting_guard():
    ...
    assert "createRuntimeProfileForm" not in js
