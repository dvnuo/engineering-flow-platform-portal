...
class K8sServiceNoopTest(unittest.TestCase):
    ...
    def test_opencode_runtime_clones_skills_and_tools_to_app_asset_dirs_without_native_runtime_mounts(self):
        ...
        self.assertIn("/app/skills", mount_paths)
        self.assertIn("/app/tools", mount_paths)
        self.assertIn("/root/.local/share/opencode", mount_paths)
        self.assertIn("/root/.local/share/efp-compat", mount_paths)
        mounts_by_path = {m.mount_path: m for m in mounts}
        self.assertEqual(mounts_by_path["/root/.local/share/opencode"].sub_path, "efp-agents/a2/opencode-state")
        self.assertEqual(mounts_by_path["/root/.local/share/efp-compat"].sub_path, "efp-agents/a2/adapter-state")
    ...
    def test_native_runtime_env_exposes_skills_and_tools_without_opencode_workspace(self):
        ...
        self.assertNotIn("OPENCODE_DATA_DIR", env_map)
    ...
    def test_patch_deployment_uses_json_merge_patch_and_replaces_opencode_lists(self):
        ...
        self.assertEqual(mount_paths, {"/workspace", "/root/.local/share/opencode", "/root/.local/share/efp-compat", "/app/tools"})
        self.assertNotIn("/app/src", mount_paths)
        self.assertNotIn("/app/.git", mount_paths)
    ...
    def test_opencode_runtime_mounts_app_tools_without_tool_repo(self):
        ...
        self.assertIn("chown -R 0:0", state_init.args[0])
