...
(async () => {{
  ...
  const caseA = {{ success: successCalls.length, incomplete: incompleteCalls.length, reason: incompleteCalls[0]?.reason || "", cleared: activeRequestCleared, active: chatState.activeRequest === reqA }};
