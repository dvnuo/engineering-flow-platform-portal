...
def test_internal_runtime_context_includes_runtime_profile_context(monkeypatch):
    ...
    try:
        ...
        db = next(db_gen)
        rp = RuntimeProfile(owner_user_id=agent.owner_user_id, name="rp-ctx", config_json='{"llm": {"provider": "openai", "model": "gpt-4", "temperature": 0.4}, "ssh": {"hack": true}}', revision=3, is_default=True)
        db.add(rp)
        db.commit()
        db.refresh(rp)
