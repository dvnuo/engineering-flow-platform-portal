...
def test_proxy_agent_blocks_session_manage_endpoints_for_non_owner(monkeypatch):
    from app.main import app
    import app.api.proxy as proxy_module
    ...
    try:
        ...
        async def _fake_forward(**kwargs):
            raise AssertionError(f"Forward should not be called for forbidden session management endpoint: {kwargs.get('subpath')}")

        monkeypatch.setattr(proxy_module.proxy_service, "forward", _fake_forward)
        client = TestClient(app)

        delete_resp = client.delete("/a/agent-1/api/sessions/s-1")
...
def test_non_allowlisted_arbitrary_header_is_not_forwarded_in_browser_proxy_allowlist():
...
def test_non_allowlisted_portal_prefixed_arbitrary_header_is_not_forwarded_in_browser_proxy_allowlist():
    from app.services.proxy_service import ProxyService

    outbound = ProxyService._build_outbound_headers(
        headers={"content-type": "application/json"},
        extra_headers={
            "X-Portal-Arbitrary-Header": "ignored",
            "X-Portal-User-Id": "10",
...
def test_proxy_agent_server_files_download_preserves_content_disposition(monkeypatch):
    from app.main import app
    import app.api.proxy as proxy_module
...
def test_proxy_agent_server_files_download_fallback_filename_when_runtime_header_missing(monkeypatch):
    from app.main import app
    import app.api.proxy as proxy_module
...
def test_proxy_agent_server_files_download_multiple_paths_fallback_filename(monkeypatch):
    from app.main import app
    import app.api.proxy as proxy_module
