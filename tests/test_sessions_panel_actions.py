...
def _setup_client(monkeypatch, user, agent):
    ...
    monkeypatch.setattr(
        ...
        lambda _db: SimpleNamespace(list_by_agent_and_session_ids=lambda **_kwargs: []),
    )
...
def test_sessions_panel_renders_context_preview_when_metadata_exists(monkeypatch):
    ...
    monkeypatch.setattr(
        web_module,
        "AgentSessionMetadataRepository",
        lambda _db: SimpleNamespace(list_by_agent_and_session_ids=lambda **_kwargs: [metadata_record]),
    )
