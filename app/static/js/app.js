...
function bindToolButtons() {
  openServerFilesBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      renderDetailToolContent("Server Files", files);
    } catch (err) {
  ...
  openMyUploadsBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      renderDetailToolContent("My Uploads", uploads);
    } catch (err) {
  ...
  openChatSettingsBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (detailToolTitle) detailToolTitle.textContent = "Settings";
      if (detailToolContent) {
        ...
        saveBtn.onclick = async () => {
          try {
            ...
            await tryProxyApi(robot.id, ["api/config/save", "api/settings/save"], {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(payload),
            });
        ...
        detailToolContent.append(editor, saveBtn);
      }
    } catch (err) {
