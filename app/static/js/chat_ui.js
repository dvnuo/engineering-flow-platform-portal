...
function renderAgentMeta(agent) {
  ...
  const dateStr = created.toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
  ...
  // Only show description if exists
  const hasDesc = agent.description && agent.description.trim();

  dom.agentMeta.innerHTML = `
    <div class="text-xs space-y-1.5">
      <div class="flex items-center gap-2">
        <span class="text-slate-400 shrink-0">Image:</span>
        <code class="text-[10px] bg-slate-100 text-slate-700 px-1 rounded truncate">${safe(agent.image)}</code>
      </div>
      <div class="flex items-center gap-2">
      ...
      <div class="flex items-center gap-2">
        <span class="text-slate-400 shrink-0">Resources:</span>
        <span class="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 text-[10px] font-medium">${cpu}</span>
        <span class="inline-flex items-center px-1.5 py-0.5 rounded bg-purple-50 text-purple-700 text-[10px] font-medium">${mem}</span>
        <span class="inline-flex items-center px-1.5 py-0.5 rounded bg-green-50 text-green-700 text-[10px] font-medium">${disk}Gi</span>
      </div>
      ${hasDesc ? `
      <div class="flex items-start gap-2">
        <span class="text-slate-400 shrink-0">Desc:</span>
        <span class="text-slate-600">${safe(agent.description)}</span>
      </div>
      ` : ''}
    </div>
  `;
...
function renderAgentActions(agent, status) {
  ...
  const buildIconBtn = ({ label, icon, classes, onClick, disabled = false }) => {
    ...
    button.type = "button";
    button.title = label;
    button.className = `p-2 rounded-lg border ${classes} disabled:opacity-40 disabled:cursor-not-allowed`;
  ...
  const actions = [
    { label: "Start", icon: "play", classes: "border-emerald-600 bg-emerald-600 hover:bg-emerald-700 text-white", disabled: !writable || !(status === "stopped" || status === "failed"), onClick: () => action(`/api/agents/${agent.id}/start`) },
    { label: "Stop", icon: "square", classes: "border-amber-500 bg-amber-500 hover:bg-amber-600 text-white", disabled: !writable || status !== "running", onClick: () => action(`/api/agents/${agent.id}/stop`) },
    { label: agent.visibility === "public" ? "Unshare" : "Share", icon: agent.visibility === "public" ? "lock-open" : "share-2", classes: "border-indigo-500 bg-indigo-500 hover:bg-indigo-600 text-white", disabled: !writable, onClick: () => action(`/api/agents/${agent.id}/${agent.visibility === "public" ? "unshare" : "share"}`) },
    { label: "Edit", icon: "pencil", classes: "border-slate-500 bg-slate-500 hover:bg-slate-600 text-white", disabled: !writable, onClick: () => openEditDialog(agent) },
    { label: "Delete Runtime", icon: "trash-2", classes: "border-slate-500 bg-slate-500 hover:bg-slate-600 text-white", disabled: !writable, onClick: () => action(`/api/agents/${agent.id}/delete-runtime`, "POST", true) },
    { label: "Destroy", icon: "flame", classes: "border-rose-600 bg-rose-600 hover:bg-rose-700 text-white", disabled: !writable, onClick: () => action(`/api/agents/${agent.id}/destroy`, "POST", true) },
  ];

  actions.forEach((cfg) => container.appendChild(buildIconBtn(cfg)));
  ...
  dom.agentActions.appendChild(container);
  renderIcons();
}
