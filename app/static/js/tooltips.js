...
(function () {
  ...
  var HINTS = [
    ...
    // Troubleshooting CLIs (nexus, splunk, appd, pgsql). Scoped before the
    ...
    ['input[name="appd_enabled"]', "Let assistants on this profile read AppDynamics health, snapshots and violations"],
    ['input[name="appd_default_instance"]', "Instance used when an appd command does not name one with --instance"],
    ['[data-action="add-instance"][data-group="appd"]', "Connect another AppDynamics controller"],
    ['[data-test-target="appd"]', "Sign in to the default controller and list its applications"],
    ['[data-instance-item="appd"] [data-field="url"]', "Controller address, for example https://appd-controller.example.com"],
    ['[data-instance-item="appd"] [data-field="account"]', "Account name shown on the controller's login page, for example customer1"],
    ['[data-instance-item="appd"] [data-field="auth_type"]', "API client exchanges a client secret for a token; Basic signs in as username@account"],
    ['[data-instance-item="appd"] [data-field="username"]', "API client name, or the username for Basic sign-in"],
    ['[data-instance-item="appd"] [data-field="password"]', "Only for Basic sign-in"],
    ['[data-instance-item="appd"] [data-field="token"]', "Client secret of the API client. Only for API client sign-in."],
    ...
    ['[data-instance-item="nexus"] [data-field="name"], [data-instance-item="splunk"] [data-field="name"], [data-instance-item="appd"] [data-field="name"], [data-instance-item="pgsql"] [data-field="name"]', "Short name for this instance. The assistant passes it as --instance."],
