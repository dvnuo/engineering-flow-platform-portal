...
def create_policy_profile(payload: PolicyProfileCreateRequest, user=Depends(get_current_user), db: Session = Depends(get_db)):
    _ = user
...
def list_policy_profiles(user=Depends(get_current_user), db: Session = Depends(get_db)):
    _ = user
...
def get_policy_profile(profile_id: str, user=Depends(get_current_user), db: Session = Depends(get_db)):
    _ = user
