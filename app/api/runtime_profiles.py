...
from app.schemas.runtime_profile import RuntimeProfileCreateRequest, RuntimeProfileResponse, RuntimeProfileUpdateRequest
...
def update_runtime_profile(profile_id: str, payload: RuntimeProfileUpdateRequest, user=Depends(require_admin), db: Session = Depends(get_db)):
    _ = user
    repo = RuntimeProfileRepository(db)
